    ...
    <main className="flex flex-col items-center justify-between pb-40 bg-black">
      <div className="absolute top-5 hidden w-full justify-between px-5 sm:flex">
        <a
          ...
          target="_blank"
          className="rounded-lg p-2 transition-colors duration-200 bg-stone-100 sm:bottom-auto"
        >
          <VercelIcon />
        </a>
        <a
        ...
        <div className="sm:mx-0 mx-5 mt-20 max-w-screen-md rounded-md sm:w-full">
          ...
          <div className="flex flex-col space-y-4 p-7 sm:p-10">
            {examples.map((example, i) => (
              <button
                key={i}
                className="rounded-md border border-gray-200 bg-black px-5 py-3 text-left text-sm text-gray-500 transition-all duration-75 hover:border-black hover:text-gray-700 active:bg-gray-50"
                onClick={() => {
                  setInput(example);
      ...
      <div className="fixed bottom-0 flex w-full flex-col items-center space-y-3 p-5 pb-3 sm:px-0">
        <form
          ...
          className="relative w-full max-w-screen-md rounded-full border border-[#525252] bg-black px-4 pb-2 pt-3 sm:pb-3 sm:pt-4"
        >
          <Textarea
            ref={inputRef}
            tabIndex={0}
            ...
            spellCheck={false}
            className="w-full pl-5 focus:outline-none bg-black text-[#ffffff80]"
          />
          <button
            className={clsx(
